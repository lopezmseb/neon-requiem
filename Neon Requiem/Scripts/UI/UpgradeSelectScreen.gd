...
func set_controller_device(device_id: int):
	# If a different controller is assigned, clear the previous controller
	if current_device_id != -1 and current_device_id != device_id and device_id != -1:
	...
	for input_name in directions.keys():
		# Remove all previous mappings for this action
